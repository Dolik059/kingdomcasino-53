...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-dbe3c02f";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-dbe3c02f">'+'<span class="tile-face-dbe3c02f tile-front-dbe3c02f"></span>'+'<span class="tile-face-dbe3c02f tile-back-dbe3c02f"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dbe3c02f")){return;}
var back=tile.querySelector(".tile-back-dbe3c02f");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-dbe3c02f");if(isMine){tile.classList.add("boom-dbe3c02f");}
if(dim){tile.classList.add("dimmed-dbe3c02f");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-dbe3c02f")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-dbe3c02f");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-dbe3c02f")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-dbe3c02f");void panelEl.offsetWidth;panelEl.classList.add("jolt-dbe3c02f");}
endRound();}
